    ...
    struct EntityDetailPlatformPresentationModifier<EditContent: View>: ViewModifier {
        ...
        func body(content: Content) -> some View {
            content
                ...
                .toolbar {
                    if let identifyAction {
                        ToolbarItem(placement: .primaryAction) {
                            Button {
                                onAction(identifyAction)
                            } label: {
                                Label(
                                    actionLabel(identifyAction),
                                    systemImage: identifyAction.systemImage
                                )
                            }
                            .disabled(!isActionEnabled(identifyAction))
                            .help(actionHint(identifyAction))
                            .accessibilityIdentifier("entity-detail.action.identify")
                        }
                    }
                }
        ...
        private var identifyAction: EntityDetailAction? {
            presentation?.modificationActions.first {
