...
final class MusicPlayerControllerTests: XCTestCase {
    ...
    func testActiveMusicReportsWallClockListeningTime() async throws {
        ...
        let update = try XCTUnwrap(service.consumptionUpdates.last)
        XCTAssertEqual(update.id, track.id)
        XCTAssertEqual(update.positionSeconds, 12)
    ...
    func testSlowCompletionReportCannotAdvanceAReplacementQueue() async {
        ...
        await service.waitUntilRecordingStarts()
    ...
    func testDiscardingSessionPlaybackStopsAndRemovesRestorationWithoutReportingProgress() async {
        ...
        XCTAssertTrue(controller.queue.tracks.isEmpty)
        XCTAssertEqual(store.clearCallCount, 1)
        XCTAssertTrue(service.consumptionUpdates.isEmpty)
    }
    ...
    func testAppWideAudiobookPlaybackReportsListeningTimeAndReaderPosition() async throws {
        ...
        controller.play(
            tracks: [track],
            context: audiobookContext(bookID: bookID, track: track)
        )
        ...
        let update = try XCTUnwrap(service.entityProgressUpdates.last)
    ...
    func testActiveAudiobookRestartsActivityAfterLifecycleFlush() async throws {
        ...
        XCTAssertEqual(
            service.entityProgressUpdates.compactMap(\.request.activitySeconds),
            [5, 10]
        )
    }

    func testPausingAudiobookFlushesTheLatestSubHeartbeatPosition() async throws {
        let bookID = UUID(uuidString: "10000000-0000-0000-0000-000000000001")!
        let track = makeTrack(idSuffix: 1, duration: 3_600)
        let service = MusicPlaybackServiceStub()
        let clock = TestMusicPlaybackClock()
        ...
        let update = try XCTUnwrap(service.entityProgressUpdates.last)
        XCTAssertEqual(update.request.index, 11)
        XCTAssertEqual(update.request.activitySeconds, 7)
        XCTAssertFalse(controller.isPlaying)
    }

    func testAudiobookBufferingTimeIsNotCountedAsListeningActivity() async {
        let bookID = UUID(uuidString: "10000000-0000-0000-0000-000000000001")!
        let track = makeTrack(idSuffix: 1, duration: 3_600)
        let service = MusicPlaybackServiceStub()
        let clock = TestMusicPlaybackClock()
...
private final class MusicPlaybackServiceStub: MusicPlaybackServicing {
    ...
    func updateEntityConsumption(
        ...
        consumptionUpdates.append((id, positionSeconds, activitySeconds, completed))
    }
...
private final class SuspendingMusicPlaybackServiceStub: MusicPlaybackServicing {
    ...
    private var recordingStartedContinuation: CheckedContinuation<Void, Never>?
    ...
    func recordEntityConsumptionEvent(
        ...
        recordingStartedContinuation?.resume()
        recordingStartedContinuation = nil
        await withCheckedContinuation { continuation in recordingContinuation = continuation }
    }
    ...
    func waitUntilRecordingStarts() async {
        guard recordingContinuation == nil else { return }
