...
- On iOS, compact toolbar and sheet-toolbar actions use familiar SF Symbols instead of visible text. Preserve the localized action title as the accessibility label and use `PrismediaToolbarActionButton` for standard confirm, cancel, close, and done actions. Menus, dialogs, forms, and prominent in-sheet content actions keep text where it carries meaning.
