...
class VLCKitProfile5PatchTests(unittest.TestCase):
    def test_glsl100_reshape_patch_is_part_of_the_tvos_build_contract(self) -> None:
        bootstrap = (ROOT / "Scripts/bootstrap-vlckit.sh").read_text()
        profile5_patch = (
            ROOT / "Scripts/Patches/VLCKit4-tvOS-DolbyVisionProfile5.patch"
        ).read_text()
        glsl_patch_path = (
            ROOT
            / "Scripts/Patches/VLCKit4-tvOS-DolbyVisionProfile5-GLSL100.patch"
        )
        ...
        self.assertIn("glsl100-dovi-reshape", bootstrap)
        self.assertIn("glsl100-dovi-reshape", profile5_patch)

    def test_adaptive_http_bearer_patch_is_part_of_the_tvos_build_contract(self) -> None:
        bootstrap = (ROOT / "Scripts/bootstrap-vlckit.sh").read_text()
        bearer_patch_path = (
        ...
        self.assertIn("adaptive HTTP bearer forwarding enabled", bootstrap)
