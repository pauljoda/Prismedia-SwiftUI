#!/usr/bin/env python3
"""Generate native Entity-kind definitions from Prismedia's backend code manifest."""
...
from entity_kind_definition_codegen import OUTPUT, render_manifest
...
def main() -> int:
    ...
    try:
        ...
        print(f"Unable to generate native Entity-kind definitions: {error}", file=sys.stderr)
        return 2

    if arguments.check:
        if current == expected:
            print("Generated native Entity-kind definitions match the backend manifest.")
            return 0
        print(
            "Generated native Entity-kind definitions are out of date. "
            "Run `python3 Scripts/generate-entity-kind-definitions.py`.",
            file=sys.stderr,
        )
        return 1
